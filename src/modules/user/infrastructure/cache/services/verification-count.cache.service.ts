...
import { VERIFICATION_COUNT_KEY } from '@user/infrastructure/cache/constants/cache-key.constant';
import { CacheService } from '@cache/services/cache.service';
import { OtpMethod } from '@user/interface/constants/otp.constant';

@Injectable()
export class VerificationCountCacheService {
  constructor(private readonly cache: CacheService) {}

  async set(method: OtpMethod, to: string, count: number): Promise<void> {
    const key = this.cache.formatCacheKey(VERIFICATION_COUNT_KEY, method, to);
    await this.cache.set(key, count, AUTH_CONSTRAINTS.VERIFICATION_COUNT_TTL);
  }
